--- STEAMODDED HEADER
--- MOD_NAME: Console Deck Seer (Stable)
--- MOD_ID: ConsoleDeckSeerStable
--- MOD_AUTHOR: [thevdude] (debugged via community support)
--- MOD_DESCRIPTION: Safe card logging without UI changes
...
local function log_upcoming_cards()
    ...
    for i = 1, math.min(MAX_CARDS_TO_SHOW, remaining) do
        ...
        local card_name = get_card_identity(card)

        -- Append card name to the row
        row = row .. card_name

        -- Add separator or start a new row
        if i % 5 == 0 or i == math.min(MAX_CARDS_TO_SHOW, remaining) then
            table.insert(output, "    " .. row .. "\n")
            row = "" -- Reset the row
        else
            row = row .. ", " -- Add separator
        end
    end
